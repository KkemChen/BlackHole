...
void kkem::Logger::add_on_output(const std::string& logger, const std::function<void(const std::string&, const int& level)>& cb) {
	auto it = _map_exLog.find(logger);
	if (it != _map_exLog.end()) {
...
bool kkem::Logger::init(const std::string& logPath, const uint32_t mode,
	...
	try {
		...
		formatter->add_flag<CustomLevelFormatterFlag>('*').set_pattern("[%Y-%m-%d %H:%M:%S.%e] %^[%*]%$ |%t| [<%!> %s:%#]: %v");
...
bool kkem::Logger::add_ExLog(const std::string& logPath, const int mode) {
	try {
		...
		formatter->add_flag<CustomLevelFormatterFlag>('*').set_pattern("[%Y-%m-%d %H:%M:%S.%e] %^[%*]%$ |%t| [<%!> %s:%#] [%n]: %v");
...
spdlog::filename_t kkem::CustomRotatingFileSink::calc_filename() {
	...
	return _log_parent_path.empty() ? spdlog::fmt_lib::format("{:%Y-%m-%d}/{}_{:%Y-%m-%d_%H-%M-%S}.log", tm,
