...
void kkem::Logger::printf_(const std::string &logger, const spdlog::source_loc &loc, LogLevel lvl, const char *fmt, ...)
{
    auto fun = [](void *self, const char *fmt, va_list al) {
        auto thiz = static_cast<Logger *>(self);
        char *buf = nullptr;
...
void kkem::Logger::set_level(const std::string &logger, LogLevel lvl)
{
...
void kkem::Logger::add_on_output(const std::function<void(const std::string &msg, const int &level)> &cb)
{
...
void kkem::Logger::add_on_output(const std::string &logger,
...
/***************/
bool kkem::Logger::init(const std::string &logPath, const uint32_t mode, const uint32_t threadCount,
...
kkem::CustomRotatingFileSink::CustomRotatingFileSink(spdlog::filename_t log_path, std::size_t max_size,
...
void kkem::CustomRotatingFileSink::add_on_output(
